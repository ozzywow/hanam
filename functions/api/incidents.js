...
/* 유형별 노출 시간(발생시각 기준, 시간). 넘으면 제외(종료시각이 미래면 유지).
   GITS 피드는 해소되면 스스로 사라지므로 시간 필터에 의존하지 않는다.
   아래 값은 '멈춘 레코드' 방지용 sanity 상한:
   - 사고·고장 등 일시적 건: 12h (정상이면 그 전에 피드에서 빠짐)
   - 공사·통제 등 계획성 건: 30일 (장기 공사가 많음) */
const MAX_AGE_H = {
  "교통사고": 12, "차량사고": 12, "사고": 12,
  "차량고장": 12, "기타돌발": 12,
  "공사": 720, "통제": 720, "도로폐쇄": 720, "행사": 720,
};
