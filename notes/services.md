...
| Pages Functions | 저장소 `functions/` (자동 인식, 설정 불필요) | `functions/api/parking.js` → `/api/parking` (스타필드 주차), `functions/api/incidents.js` → `/api/incidents` (GITS 돌발정보). 스테이징(GitHub Pages)엔 없음 |
...
| GITS 돌발정보 | https://gits.gg.go.kr/web/map/webLoadINCIData.do | 실시간 사고·공사·통제. CORS 없음 → `functions/api/incidents.js` 서버측 대행 |
