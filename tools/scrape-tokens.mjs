/* docs/assets/js/regions/<region>.js 에 있는 모든 카메라의 재생 토큰과 좌표를
   GITS 에서 다시 긁어 비교/갱신. (지역 파일이 여러 개여도 전부 순회)

   사용:
     node tools/scrape-tokens.mjs           # 확인만 (변경분 표시)
     node tools/scrape-tokens.mjs --write   # 각 regions/*.js 의 url·lat·lng 를 최신값으로 교체
...
const REGIONS_DIR = resolve(HERE, "../docs/assets/js/regions");
...
const regionFiles = readdirSync(REGIONS_DIR)
  .filter(f => f.endsWith(".js"))
  .map(f => resolve(REGIONS_DIR, f));
if (!regionFiles.length){ console.error(`${REGIONS_DIR} 에서 지역 파일(.js)을 찾지 못했습니다.`); process.exit(1); }
...
// 팝업 응답 캐시 — 같은 id 가 여러 지역/덱에 중복 등장해도 한 번만 요청
const popupCache = new Map();
async function fetchFresh(cam){
  if (!popupCache.has(cam.id)){
    try {
      const r = await fetch(POPUP(cam.id), { headers: HEADERS });
      popupCache.set(cam.id, await r.text());
    } catch (e) {
      popupCache.set(cam.id, null);
      console.log(`  ✗ ${cam.id}  요청 실패 (${e.message})`);
    }
  }
  const html = popupCache.get(cam.id);
  if (html == null) return undefined;

  if (cam.type === "hls"){
    const m = html.match(/(?:https?:)?\/\/gitsview\.gg\.go\.kr\/\d+\/[^"!]+!hls/);
    if (m) return "https:" + m[0].replace(/^https?:/, "");
  } else {
...
let totChanged = 0, totFailed = 0, totSame = 0;

for (const CAMS_PATH of regionFiles){
  let src = readFileSync(CAMS_PATH, "utf8");
  const cams = [...src.matchAll(camRe)].map(m => ({ id: m[1], type: m[2], url: m[3], block: m[0] }));
  const label = CAMS_PATH.split(/[\\/]/).slice(-1)[0];
  if (!cams.length){ console.log(`\n[${label}] 카메라 없음 — 건너뜀`); continue; }

  console.log(`\n[${label}] ${cams.length}개 카메라 확인 중…`);
  let changed = 0, failed = 0;

  for (const cam of cams){
    const fresh = await fetchFresh(cam);
    if (!fresh){
  ...
  console.log(`  → 변경 ${changed} · 실패 ${failed} · 동일 ${cams.length - changed - failed}`);
  if (changed && WRITE){ writeFileSync(CAMS_PATH, src); console.log(`  → ${label} 갱신 완료`); }
  totChanged += changed; totFailed += failed; totSame += cams.length - changed - failed;
}

console.log(`\n합계 — 변경 ${totChanged} · 실패 ${totFailed} · 동일 ${totSame}`);
if (totChanged && !WRITE) console.log("→ 반영하려면 --write 로 다시 실행");
